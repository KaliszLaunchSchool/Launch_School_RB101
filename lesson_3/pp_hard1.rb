...
Nothing. Greeting evaluates to true so the code is not run
LS solution:
...
variable within an if block, even if that if block doesn’t get executed, the local 
...
# greetings should still be the hash { a: 'hi' } as it is reassigned to 
# informal_greeting prior to its mutation
WRONG
LS Solution
The output is {:a=>"hi there"}. The reason is because informal_greeting is a 
reference to the original object. The line informal_greeting << ' there' was using 
the String#<< method, which modifies the object that called it. This means that the 
original object was changed, thereby impacting the value in greetings. If instead of 
modifying the original object, we wanted to only modify informal_greeting but not 
greetings, there are a couple of options:
...
def dot_separated_ip_address?(input_string)
  ...
  else puts "Please enter a valid IP address with 4 components"
  end
  return true
end

#LS solution
There are several ways to fix this. To determine if there are exactly 4 dot-separated 
"words" in the string, you can simply add a check for dot_separated_words.size after 
splitting the string.

The other error in Ben's code is that instead of returning false upon encountering 
a non-numeric component, he used break to break out of the while loop. Once he breaks, 
control falls through to the return true statement. He can fix this by performing 
...
Note: Ben's original return true on the last line of the method can be shortened to 
just true. This is because Ruby returns the result of the last evaluated expression.
=end
