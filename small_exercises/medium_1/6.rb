...
28, is left in the register. If we do another MULT at this point, then the stack is transformed to 3, and the 
...
NUMBERS = ('-1'..'-1000').to_a + ('0'..'1000').to_a

def commands(command, register, stack)
  if command == 'PRINT'
    register.each {|item| p item}
  elsif command == 'PUSH'
    stack << register.sum
  elsif command == 'ADD'
     register << stack.pop
     register.replace([register.sum])
  elsif command == 'SUB'
    num = stack.pop
    register << (num *= -1)
    register.replace([register.sum])
    ...
    num = stack.pop
    register_value = register.sum
    register.replace([num * register_value])
  elsif command == 'DIV'
    num = stack.pop
    register_value = register.sum
    register.replace([register_value / num])
  elsif command == 'MOD'
    num = stack.pop
    register_value = register.sum
    register.replace([register_value % num])
  elsif command == 'POP'
    register.replace([stack.pop])
  else register.replace([command.to_i])
  end
end

def minilang(command_line)
  register = [0]
  stack = []
  command_line.split(' ').each do |string|
    commands(string, register, stack)
  end
end
...
Discussion
This exercise takes a lot of time to explain, but is not that difficult to implement. Once you understand what 
is expected, it's simply a matter of setting up a processing loop, and doing the necessary processing.

We start by creating our stack and register, and then just walk through each "token" in the program, performing 
the appropriate action during each iteration. A token is one of the words or values in the language.

We use a case statement for the body of our loop. The one thing that might trip you up is remembering to put 
integer tokens into the register; an easy step to do, an easy step to forget.
...
Copy Code
(3 + (4 * 5) - 7) / (5 % 3)
The answer should be 8. This is trickier than it sounds! Note that we aren't asking you to modify the #minilang 
method; we want you to write a Minilang program that can be passed to #minilang and evaluated.

Add some error handling to your method. In particular, have the method detect empty stack conditions, and 
invalid tokens in the program, and report those. Ideally, the method should return an error message if an 
error occurs, and nil if the program runs successfully.
=end